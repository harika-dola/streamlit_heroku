...
if len(choice) > 0:
    modified_data = data[data['noted_date'].dt.month == month]
    fig_choice1=px.histogram(modified_data, x='out/in', y='temp', histfunc='count',   height=600, width=800)
    st.plotly_chart(fig_choice1)
    fig_choice=px.histogram(modified_data, x='out/in', y='temp', histfunc='count', color='temp',   height=600, width=800)
